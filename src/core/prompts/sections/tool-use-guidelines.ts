...
export function getToolUseGuidelinesSection(codeIndexManager?: CodeIndexManager): string {
	...
	if (isCodebaseSearchAvailable) {
		...
		guidelinesList.push(
			...
			`${itemNumber++}. Choose the most appropriate tool based on the task and the tool descriptions provided.`,
		)
	} else {
		guidelinesList.push(
...
Proceed step-by-step, waiting for the user's message after each XML tool use before moving forward with the task. This approach allows you to:
